...
/* the jet colormap */
static struct {unsigned char r,g,b;} jet[]=
  {
   {0  ,   0  , 132},
   {0  ,   0  , 136},
   {0  ,   0  , 140},
	...
	scale = 1.0 + gain + log10 ((double) exposure_time_us / 10000.0);
        ...
        switch (pixel_format)
          ...
          case ARV_PIXEL_FORMAT_MONO_8:
            for (y = 0; y < height; y++)
              for (x = 0; x < width; x++) {
                unsigned char* pixel = ((unsigned char *) buffer->priv->data)+y * width + x;
          ...
          case ARV_PIXEL_FORMAT_RGB_8_PACKED:
             for (y = 0; y < height; y++)
              for (x = 0; x < width; x++) {
                unsigned char* pixel = ((unsigned char *) buffer->priv->data) + 3*(y * width + x);
                pixel_value = (x + buffer->priv->frame_id + y) % 255;
                pixel_value *= scale;

                if (pixel_value < 0.0) pixel_value = 0;
                else if (pixel_value > 255.0) pixel_value = 255;

                pixel[0] = jet[(unsigned)pixel_value].r;
                pixel[1] = jet[(unsigned)pixel_value].g;
                pixel[2] = jet[(unsigned)pixel_value].b;
              }
           
          default:
            break; //TODO log a warning message
          }
}
