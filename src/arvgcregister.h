...
struct _ArvGcRegisterInterface {
	...
	void 		(*get)			(ArvGcRegister *gc_register, void *buffer, guint64 length,
                                                 GError **error);
	void 		(*set)			(ArvGcRegister *gc_register, const void *buffer, guint64 length,
                                                 GError **error);
...
ARV_API void		arv_gc_register_get		(ArvGcRegister *gc_register, void *buffer, guint64 length,
                                                         GError **error);
ARV_API void		arv_gc_register_set		(ArvGcRegister *gc_register, const void *buffer, guint64 length,
                                                         GError **error);
