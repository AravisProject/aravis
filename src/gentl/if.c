...
GC_API IFGetInfo               ( IF_HANDLE hIface, INTERFACE_INFO_CMD iInfoCmd, INFO_DATATYPE *piType, void *pBuffer, size_t *piSize ){
	...
	switch(iInfoCmd){
		...
		case INTERFACE_INFO_TLTYPE:
			return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_protocol(hIface,/*unused*/0),piSize,piType);
		default:
...
GC_API IFGetDeviceInfo         ( IF_HANDLE hIface, const char *sDeviceID, DEVICE_INFO_CMD iInfoCmd, INFO_DATATYPE *piType, void *pBuffer, size_t *piSize ){
	...
	for(i=0; i<arv_interface_get_n_devices(hIface); i++){
		if(g_strcmp0(arv_interface_get_device_id(hIface,i),sDeviceID)==0){ iDev=i; break; }
	}
	...
	#endif
	switch(iInfoCmd){
		case DEVICE_INFO_ID:     return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_id(hIface,iDev),piSize,piType);
		case DEVICE_INFO_VENDOR: return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_vendor(hIface,iDev),piSize,piType);
		case DEVICE_INFO_MODEL:  return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_model(hIface,iDev),piSize,piType);
		case DEVICE_INFO_TLTYPE: return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_protocol(hIface,iDev),piSize,piType);
		case DEVICE_INFO_DISPLAYNAME:   return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,/* ?? */arv_interface_get_device_id(hIface,iDev),piSize,piType);
		case DEVICE_INFO_USER_DEFINED_NAME:  return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,/* ?? */arv_interface_get_device_id(hIface,iDev),piSize,piType);
		case DEVICE_INFO_SERIAL_NUMBER: return gentl_to_buf(INFO_DATATYPE_STRING,pBuffer,arv_interface_get_device_serial_nbr(hIface,iDev),piSize,piType);
...
GC_API IFOpenDevice            ( IF_HANDLE hIface, const char *sDeviceID, DEVICE_ACCESS_FLAGS iOpenFlag, DEV_HANDLE *phDevice ){
	ArvCamera* cam;
	_IF_CHECK_HANDLE;
	arv_trace_gentl("%s (hIface=%s[%p], sDeviceID=%s, iOpenFlag=%d)",__FUNCTION__,G_OBJECT_TYPE_NAME(hIface),hIface,sDeviceID,iOpenFlag);
	cam=arv_camera_new(sDeviceID,&gentl_err);
	if(!cam) return GC_ERR_IO;
	*phDevice=cam;
	return GC_ERR_SUCCESS;
}
/* GenTL v1.4 */
