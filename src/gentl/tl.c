...
GC_API TLOpen                  ( TL_HANDLE *phTL ){
	...
	GENTL_ENSURE_INIT;
	if(phTL==NULL) return GC_ERR_INVALID_PARAMETER;
	if(gentl_transport_layer!=NULL) return GC_ERR_RESOURCE_IN_USE;
...
GC_API TLGetInfo               ( TL_HANDLE hTL, TL_INFO_CMD iInfoCmd, INFO_DATATYPE *piType, void *pBuffer, size_t *piSize ){ GENTL_NYI; }
...
GC_API TLGetNumInterfaces      ( TL_HANDLE hTL, uint32_t *piNumIfaces ){
	...
	_TL_CHECK_HANDLE;
	if(!piNumIfaces) return GC_ERR_INVALID_PARAMETER;
...
GC_API TLGetInterfaceID        ( TL_HANDLE hTL, uint32_t iIndex,  char *sID, size_t *piSize ){
	...
	_TL_CHECK_HANDLE;
	if(iIndex>=arv_get_n_interfaces()) return GC_ERR_INVALID_INDEX;
...
GC_API TLGetInterfaceInfo      ( TL_HANDLE hTL, const char *sIfaceID, INTERFACE_INFO_CMD iInfoCmd, INFO_DATATYPE *piType, void *pBuffer, size_t *piSize ){
	...
	if(!iface) return GC_ERR_INVALID_ID;
	switch(iInfoCmd){
...
GC_API TLOpenInterface         ( TL_HANDLE hTL, const char *sIfaceID, IF_HANDLE *phIface ){
	...
	_TL_CHECK_HANDLE;
	if(sIfaceID==NULL || phIface==NULL) return GC_ERR_INVALID_PARAMETER;
	iface=arv_get_interface_by_id(sIfaceID);
	if(!iface) return GC_ERR_INVALID_ID;
	*phIface=(void*)iface;
