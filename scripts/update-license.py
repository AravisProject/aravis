...
new_header_content = """SPDX-License-Identifier: LGPL-2.1-or-later
 *
...
pattern = re.compile(r"SPDX-License-Identifier:.*?\*([\s\S]*?)\*\s*Author", re.DOTALL)
...
def process_directory(directory):
    for filename in os.listdir(directory):
        if filename.endswith('.h') or filename.endswith('.c') or filename.endswith('.h.in'):
